...
一个 Claude Code skill,做一件事:**让临床科研论文读起来像是课题组自己写的,不是基础科学腔,也不是 AI 腔。**
...
| **C — 整篇打磨** | 确定研究类型和体裁 → 按 IMRaD 拆单元 → 逐单元改(各节可并行)→ 强制跨节金线校验 → 三种交付物。 | 全稿精修,投稿前。 |
...
**教的是原理,不是模板。** 旧版 phrasebank 把范文拆成填空框让人往里填——结果就是 "Nearly one in five patients experienced X" 这种复读机。新版彻底推翻了:39 条招式(Title 到 Conclusion),每条给的是**原理 + 范文原句(grep 回原文逐条核实过) + AI 腔反例 + 防重复提示**,不留任何可粘贴的空格句。
...
**该引用别人的不自己写。** 通用 IMRaD 结构、hedging 体系、时态、CONSORT / STROBE / TRIPOD 报告规范 —— 交给 `academic-research-skills` 插件和 `scientific-writing` skill。词法层去 AI 交给 `humanizer_academic`(带临床豁免:Methods 被动不动、破折号按阈值不按零容忍、hedging 绑 section)。本 skill 只做别处没有的:**这个课题组到底怎么写。**
...
**查草稿:** `python scripts/check_draft.py draft.txt --genre A`

**投稿前跑清单审计:**
```
grep -A10 "#mapping"   references/reporting_checklists.md   # 找到对应清单
grep -A30 "#strobe"    references/reporting_checklists.md   # 或 #consort / #stard / #tripod
```
